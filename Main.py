...
def knn(dataset, k):
    ...
    '''
    neigh_dis, neigh_ind = NearestNeighbors(n_neighbors=k).fit(dataset).kneighbors(dataset, return_distance=True)
...
def cluster_with_parameters(dataset, labels_true, k, l):
    # KNN
   ...
   #print('MKNN,run_time0:', end_time0 - start_time0)
    ...
    ARI, NMI = ARI_NMI
    purity = purity_score(labels_true, final_cluster[1])
    # FILE_name = file+"_k_"+str(k)+"_K_"+str(l)+'.txt'
    # prelabel = final_cluster[1]
    # with open(FILE_name, 'w') as f:
    #     for i,item in enumerate(prelabel):
    #         f.write(str(item))
    #         if i<len(prelabel) - 1:
    #             f.write(',')
    result = f"k={k}, l={l}, ARI: {ARI}\n" \
             f"k={k}, l={l}, NMI: {NMI}\n" \
             f"k={k}, l={l}, purity: {purity}\n"

    return ARI, NMI, result, purity

if __name__ == '__main__':
    file = 'datasets/multi_dim/pendigits'
    fileName = file + '.in'
    dataset = genfromtxt(fileName, delimiter='\t')[:, 1:]
    labels_true = genfromtxt(fileName, delimiter='\t')[:, 0]
    ARI, NMI, result, purity = cluster_with_parameters(dataset, labels_true, k=10,l=19)
